...
public class NextFit {
	...
	public void run() {
		...
		for(int i = 1; i < MINUTES; i++) {
			...
			while(freeSpace >= SMALLEST_SIZE && j < process.size()) {
				int[] bestFitBlock = nextFitEmptyBlock(process.get(j).getSize());
	...
	/**
	 * Returns the Next Fit Empty block size, and the index of the Next Fit Empty Block
	 * @return index of the Next Fit Empty block.
	 */
	private int[] nextFitEmptyBlock(int x) {
		...
		System.out.println("index stopped at: "+indexStoppedAt);*/
		...
		if(firstTime){
			...
			for(int i=0; i<emptyBlocks; i++){
				if(x<=blockSize[i]){
					...
					indexStoppedAt = blockIndex[i] + x;
				}
			}
		}
		
		foundInSecondHalf = false;
		
		for(int i=0; i<emptyBlocks; i++){
			if(indexStoppedAt <= blockIndex[i] && x<= blockSize[i]){
				result[0] = blockSize[i];
				result[1] = blockIndex[i];
				indexStoppedAt = blockIndex[i] + x;
